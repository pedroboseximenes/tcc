...
class BiLstmRunner:
    ...
    def run(self):
        ...
        plot.gerar_plot_dois_eixo(eixo_x=y_true_mm, eixo_y=y_pred_mm, titulo=f"TEST [{self.index}] - bilstm{self.titulo}_lookback={self.lookback}_neuronios={self.hidden_dim}_camada={self.layer_dim}_lr={self.lr}_droprate={self.drop_rate}", xlabel="Amostra", ylabel="Chuva", legenda=['Real', 'Previsto'],dataset=self.titulo,index=self.index)
...
def rodarBILSTM(timeseries, colunas_normalizar, device, scaler, ts_scaled_df, n_test, index,  titulo, lookback, hidden_dim, layer_dim, learning_rate, drop_rate):
    ...
    nome = f'Exec{index}_Bilstm_lback={bilstm.lookback}_num_neur={bilstm.hidden_dim}_camadas={bilstm.layer_dim}_lr={bilstm.lr}_drop={bilstm.drop_rate}'
