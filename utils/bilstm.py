...
class BILSTM(ModeloBase):
    def run(self, index):
        self.logger.info("=" * 90)
        self.logger.info(f"Iniciando script BILSTM (PyTorch) {self.base_dados} com suporte a GPU e logs detalhados.")
        self.logger.info("="*90)
        self.logger.info(
        ...
        plot.gerar_plot_dois_eixo(eixo_x=y_true_mm, eixo_y=y_pred_mm, titulo=f"TEST [{index}] - bilstm{self.base_dados}_lookback={self.lookback}_neuronios={self.config['hidden_dim']}_camada={self.config['layer_dim']}_lr={self.config['learning_rate']}_droprate={self.config['drop_rate']}", xlabel="Amostra", ylabel="Chuva", legenda=['Real', 'Previsto'],dataset=self.base_dados,index=index)
